...
public class TFIDFModelProvider implements Provider<TFIDFModel> {
    ...
    /**
     * Construct a model builder.  The {@link Inject} annotation on this constructor tells LensKit that it can be used
     * to build the model builder.
     *
    ...
    public TFIDFModel get() {
        ...
        for (long item : items) {
            ...
            for (Entity tagApplication : dao.query(TagData.ITEM_TAG_TYPE)
                ...
                if (!work.containsKey(tag)) {
                    // not already added to the work map...tag not counted in docfreq yet
                    double currentDocCount = docFreq.getOrDefault(tag, 0.0);
                    docFreq.put(tag, currentDocCount + 1.0);
                }

                double currentTagCount = work.getOrDefault(tag, 0.0);

                work.put(tag, currentTagCount + 1.0);
        ...
        for (Map.Entry<Long, Map<String, Double>> entry : itemVectors.entrySet()) {
            ...
            for (Map.Entry<String, Double> tfValues : tv.entrySet()) {
                String tagName = tfValues.getKey();
                double tagFreq = tfValues.getValue();
                double tfidfValue = tagFreq * docFreq.get(tagName);
                tfValues.setValue(tfidfValue);
                sumOfSquares += Math.pow(tfidfValue, 2);
            }
            Double normDenominator = Math.sqrt(sumOfSquares);

            for (Map.Entry<String, Double> tfValues : tv.entrySet()) {
                double oldValue = tfValues.getValue();
                double normValue = oldValue / normDenominator;
                tfValues.setValue(normValue);
