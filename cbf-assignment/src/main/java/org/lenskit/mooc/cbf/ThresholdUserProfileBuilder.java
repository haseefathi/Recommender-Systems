    ...
    public Map<String, Double> makeUserProfile(@Nonnull List<Rating> ratings) {
        ...
        for (Rating r : ratings) {
            if (r.getValue() >= RATING_THRESHOLD) {
                ...
                Map<String, Double> movieTagVector = model.getItemVector(movieId);
                for (Map.Entry<String, Double> tagVector : movieTagVector.entrySet()) {
                    String tagName = tagVector.getKey();
                    double tagValue = tagVector.getValue();
                    if (!profile.containsKey(tagName)) {
                        profile.put(tagName, tagValue);
                    } else {
                        double currentValue = profile.get(tagName);
                        profile.put(tagName, currentValue + tagValue);
                    }
                }
            }
        }
