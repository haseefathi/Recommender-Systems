...
public class MeanItemBasedItemRecommender extends AbstractItemBasedItemRecommender {
     ...
     * {@inheritDoc}
     * <p>
     * This is the LensKit recommend method.  It takes several parameters; we implement it for you in terms of a simpler
     * method ({@link #recommendItems(int, LongSet)}).
     */
    @Override
    ...
    private ResultList recommendItems(int n, LongSet items) {
        ...
        while (iterator.hasNext()) {
            ...
            try {
                double averageScore = model.getMeanRating(movieID);
                Result result = Results.create(movieID, averageScore);
                results.add(result);

            } catch (IllegalArgumentException e) {
                logger.info("The movie {} has no average score", movieID);
            }
        }
